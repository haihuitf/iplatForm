...
class ProjectInfoManager(models.Manager):
    ...
    def update_project(self, id, project_name, responsible_name, test_user, dev_user, publish_app, simple_desc,
                       other_desc):
        '''
        项目模块内容更新
        :param id:
...
class TestCaseInfoManager(models.Manager):
    ...
    def insert_case(self, modules, **kwargs):
        ...
        self.create(name=kwargs.get('test').get('name'), belong_project=case_info.pop('belong_project_id'),
                    belong_module_id=modules, author=case_info.pop('author'), include=case_info.pop('include'),
                    request=kwargs)
